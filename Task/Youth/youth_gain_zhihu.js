...
if (!$.isNode() && !lookbodys.indexOf("&") == -1) {
  ...
  if ($.isNode()) {
    if (process.env.YOUTH_START_ZHIHU && process.env.YOUTH_START_ZHIHU.indexOf('&') > -1) {
      StartBody = process.env.YOUTH_START_ZHIHU.split('&');
    } else {
      StartBody = [process.env.YOUTH_START_ZHIHU]
    };
    if (process.env.YOUTH_LOOK_ZHIHU && process.env.YOUTH_LOOK_ZHIHU.indexOf('&') > -1) {
      LookBody = process.env.YOUTH_LOOK_ZHIHU.split('&');
    } else {
      LookBody = [process.env.YOUTH_LOOK_ZHIHU]
    }
  }
