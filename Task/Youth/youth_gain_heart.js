...
if (!$.isNode() && !lookbodys.indexOf("&") == -1) {
  ...
  if ($.isNode()) {
    if (process.env.YOUTH_START_HEART && process.env.YOUTH_START_HEART.indexOf('&') > -1) {
      StartBody = process.env.YOUTH_START_HEART.split('&');
    } else {
      StartBody = [process.env.YOUTH_START_HEART]
    };
    if (process.env.YOUTH_LOOK_HEART && process.env.YOUTH_LOOK_HEART.indexOf('&') > -1) {
      LookBody = process.env.YOUTH_LOOK_HEART.split('&');
    } else {
      LookBody = [process.env.YOUTH_LOOK_HEART]
    }
  }
