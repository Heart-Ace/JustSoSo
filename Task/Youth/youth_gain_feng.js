...
if (!$.isNode() && !lookbodys.indexOf("&") == -1) {
  ...
  if ($.isNode()) {
    if (process.env.YOUTH_START_FENG && process.env.YOUTH_START_FENG.indexOf('&') > -1) {
      StartBody = process.env.YOUTH_START_FENG.split('&');
    } else {
      StartBody = [process.env.YOUTH_START_FENG]
    };
    if (process.env.YOUTH_LOOK_FENG && process.env.YOUTH_LOOK_FENG.indexOf('&') > -1) {
      LookBody = process.env.YOUTH_LOOK_FENG.split('&');
    } else {
      LookBody = [process.env.YOUTH_LOOK_FENG]
    }
  }
